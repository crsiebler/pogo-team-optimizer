...
- `TeamOptimizer.optimize(workers=...)` uses the existing single-process restart loop for `workers=1`; process-based parallelism should split restart batches with deterministic per-worker seeds, cap worker counts via `MAX_OPTIMIZER_WORKERS`, and reduce results through `_comparison_key()` in the parent.
